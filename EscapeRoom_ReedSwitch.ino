/*
Project:  Reed Switch with MQTT und OTA update for an Escpare Room
Author:   Thomas Edlinger for www.edistechlab.com
Date:     Created 26.11.2021
Version:  V1.0
IDE:      Arduino IDE 1.8.16
 
Required Board (Tools -> Board -> Boards Manager...)
 - Board: esp8266 by ESP8266 Community   V3.0.2 
 - Board: esp32   by Espressif Systems   V1.0.6

Required libraries (sketch -> include library -> manage libraries)
 - PubSubClient by Nick ‘O Leary V2.8.0
 - ArduinoOTA by Juraj Andrassy V1.0.5          
*/

#include <PubSubClient.h>
#include <ArduinoOTA.h>
//#include <WiFi.h>  // ESP32 only

// ----------- Code Anpassungen -----------
#define wifi_ssid "Your_SSID"
#define wifi_password "Your_Password"
#define mqtt_server "MQTT_Server_IP"
#define mqtt_user "MQTT_username"         
#define mqtt_password "MQTT_PW"
#define ESPHostname "Reed-Switch01"
String clientId = "Reed-Switch01-"; 
#define status_topic "reed_status_01"
#define timer_topic "timer_01"
#define reset_topic "reset_01"
#define activ_topic "activ_01"
#define alarm_topic "alarm_01"
const int reedPin = 5;  // D1 - optional zum anpassen
int timeout = 3600;  // 60 Minuten für das timeout in Sekunden

// ---- Ab hier muss nichts mehr angepasst werden
bool switchState;
bool reedStatusOpen;
bool activeBox;
bool timer = false;
bool errorFlag = false;
int sekunden;
const int sensorTakt = 1000; //alle 1 Sekunden wird der Sensor ausgelesen
long lastMsg = 0;
char msg[50];

WiFiClient espClient;  
PubSubClient client(espClient);  

void setup() {
  Serial.begin(115200);
  setup_wifi();
  ArduinoOTA.setHostname(ESPHostname);
  // ArduinoOTA.setPassword("admin");
  ArduinoOTA.begin();
  client.setServer(mqtt_server, 1883); 
  client.setCallback(callback); 
}

void loop() {
  if (!client.connected()) {
    reconnect();
  }
  client.loop();
  ArduinoOTA.handle(); 
  long now = millis();
  if (now - lastMsg > sensorTakt) {   
    lastMsg = now;
    readReedSwitch();
    if (timer == true) {sendTime();}
  }
}

void readReedSwitch() {
  switchState = digitalRead(reedPin);
  if (switchState == LOW) {
    if (reedStatusOpen == true) {
      Serial.println("Reed Switch is Closed.");
      client.publish(status_topic, "LOW");
      reedStatusOpen = false;
    }
  } 
  else {
    if (reedStatusOpen == false) {
      Serial.println("Reed Switch is Open.");
      client.publish(status_topic, "HIGH");
      reedStatusOpen = true;
      activSet();
      if (timer == false) {startTimer();}
    }
  } 
}

void resetBox() {
  switchState = digitalRead(reedPin);
  deactivSet();
  errorCheck();
  sekunden=0;
  timer = false;
  errorCheck();
  client.publish(timer_topic, String(sekunden).c_str(), true);
  if (switchState == LOW) {
    client.publish(status_topic, "LOW");
    reedStatusOpen = false;
  }
  else {
    client.publish(status_topic, "HIGH");
    reedStatusOpen = true;
  }
}

void startTimer() {
  timer = true;
  sekunden = 0;
}

void stopTimer() {
  timer = false;
  sekunden = 0;
}

void sendTime() {
  if (sekunden <= timeout) {
    sekunden++;
    client.publish(timer_topic, String(sekunden).c_str(), true);
  } 
  else {alarmSetON();}
}

void activSet() {
  activeBox = true;
  client.publish(activ_topic, "HIGH");
}

void deactivSet() {
  activeBox = false;
  client.publish(activ_topic, "LOW");
  stopTimer();
}

void alarmSetON() {
  errorFlag = true;
  client.publish(alarm_topic, "HIGH");
}

void alarmSetOFF() {
  errorFlag = false;
  client.publish(alarm_topic, "LOW");
}

void errorCheck() {
  if (reedStatusOpen == false) {alarmSetOFF();}
}

void setup_wifi() {
  delay(10);
  // We start by connecting to a WiFi network
  Serial.println();
  Serial.print("Connecting to ");
  Serial.println(wifi_ssid);
  WiFi.begin(wifi_ssid, wifi_password);
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }
  Serial.println("");
  Serial.println("WiFi connected");
  Serial.println("IP address: ");
  Serial.println(WiFi.localIP());
}  

void callback(char* topic, byte* message, unsigned int length) {
  Serial.print("Message arrived on topic: ");
  Serial.print(topic);
  Serial.print(". Message: ");
  String messageTemp;
  for (int i = 0; i < length; i++) {
    Serial.print((char)message[i]);
    messageTemp += (char)message[i];
  }
  Serial.println();
  if (String(topic) == reset_topic) {
    resetBox();
  }
}

 void reconnect() {
  // Loop until we're reconnected
  while (!client.connected()) {
    Serial.print("Attempting MQTT connection...");
    // Create a random client ID
    clientId += String(random(0xffff), HEX);
    // Attempt to connect
    if (client.connect(clientId.c_str(), mqtt_user, mqtt_password)) {
      Serial.println("connected");
      // Once connected, publish an announcement...
      client.publish(status_topic, ESPHostname);
      // ... and resubscribe
      client.subscribe(reset_topic);
    } else {
        Serial.print("failed, rc=");
        Serial.print(client.state());
        Serial.println(" try again in 5 seconds");
        // Wait 5 seconds before retrying
        delay(5000);
      }
   }
}
